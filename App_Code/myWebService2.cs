...
public class myWebService2 : System.Web.Services.WebService
{
    ...
    public List<object> GetCarAmount()
    {
        ...
        int eTagCar = (int)dbaccess.executeScalar("SELECT Count(*) FROM eTagCarArea WHERE ReaderID = 5");
    ...
    [WebMethod]
    public void CamRecord211(string CamId, string plate, string dtime)
    {
        Thread.Sleep(600);
        //var CamId = HttpContext.Current.Request.Form["CamId"];
        //var plate = HttpContext.Current.Request.Form["plate"];
        //var dtime = HttpContext.Current.Request.Form["dtime"];
        //Console.WriteLine(CamId + ":" + plate + "_" + dtime);
        //System.Threading.Thread.Sleep(1000);
        DateTime datetrig = DateTime.Parse(dtime.Replace("GMT",""));
        DateTime date_sub = datetrig.AddSeconds(-5);
        DataTable dt0 = dbaccess.query("SELECT * FROM u_Photos_v2 WHERE CamID='211' AND plate='" + plate + "' AND dtime >='" + date_sub.ToString("yyyy/MM/dd HH:mm:ss") + "'");
        if (dt0 != null)
        {
            if (dt0.Rows.Count > 0)
            {
        ...
        var url = "http://192.168.0.200/get_search_info?list=log";
        Uri myUri = new Uri(url);
        var request = WebRequest.Create(url);
        request.Headers.Add("Authorization", "Basic " + Convert.ToBase64String(new ASCIIEncoding().GetBytes("admin:500Net500Net")));
        ...
        string msg = reader.ReadToEnd();
            ...
            try
            {
                LPRLOG lprlog = JsonConvert.DeserializeObject<LPRLOG>(msg);
                lprinfo new_info = lprlog.INFORMATION.Where(p => p.LPR == plate).ToList().First();

                byte[] data;
                using (WebClient client = new WebClient())
                {
                    data = client.DownloadData(@"http://192.168.0.200/" + new_info.LP_BMP);
                }

                if (!Directory.Exists(@"E:/pics/211/" + datetrig.ToString("yyyyMMdd")))
                {
                    Directory.CreateDirectory(@"E:/pics/211/" + datetrig.ToString("yyyyMMdd"));
                }
                File.WriteAllBytes(@"E:/pics/211/" + datetrig.ToString("yyyyMMdd") + @"/" + datetrig.ToString("HHmmss") + "_" + plate + ".png", data);
        ...
        dbaccess.excute("INSERT INTO u_Photos_v2(CamID,dtime,plate,URI) Values('211','" + datetrig.ToString("yyyy/MM/dd HH:mm:ss") + "','" + plate + "','/pics/211/" + datetrig.ToString("yyyyMMdd") + @"/" + datetrig.ToString("HHmmss") + "_" + plate + ".png" + "')");
    ...
    public class LPRLOG
    {
        public int LPR_COUNT { get; set; }
        public lprinfo[] INFORMATION { get; set; }
    }
    public class lprinfo
    {
        public string LPR { get; set; }
        public string LP_BMP { get; set; }
    }
