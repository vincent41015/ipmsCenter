    ...
    public class XSLXHelper
    {
        ...
        public XLWorkbook Export<T>(List<T> data)
        {
            ...
            foreach (var item in typeof(T).GetProperties())
            {
                ...
                //DescriptionAttribute description = item.GetCustomAttributes(typeof(DescriptionAttribute));
                Object[] description = item.GetCustomAttributes(true);
                if (description.Count() != 0)
                {
                    //sheet.Cell(1, colIdx++).Value = description.Description;
                    sheet.Cell(1, colIdx++).Value = ((DescriptionAttribute)description[0]).Description;
                    continue;
                }
